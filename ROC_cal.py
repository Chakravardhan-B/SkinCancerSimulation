...
import os
from testing import test_model, test_SavedModel
...
tpr, fpr, auc = test_SavedModel(model, 'cuda', val_loader)

auc = [auc]

np_log = np.array(tpr, dtype=float)
np_log2 = np.array(fpr, dtype=float)
np_log3 = np.array(auc, dtype=float)
if not os.path.exists('./results/ROC/HAM10000/' + network + '/Test/'):
    os.makedirs('./results/ROC/HAM10000/' + network + '/Test/')
loc = './results/ROC/HAM10000/' + network + '/Test/' + 'TPR.csv'
loc2 = './results/ROC/HAM10000/' + network + '/Test/' + 'FPR.csv'
loc3 = './results/ROC/HAM10000/' + network + '/Test/' + 'AUC.csv'
np.savetxt(loc, np_log, delimiter=',', fmt='%.6f')
np.savetxt(loc2, np_log2, delimiter=',', fmt='%.6f')
np.savetxt(loc3, np_log3, delimiter=',', fmt='%.6f')
